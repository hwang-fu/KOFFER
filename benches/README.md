...
`crates/cryptography/examples/ct_decap.rs` is a timing-leakage harness for ML-KEM decapsulation, using the dudect method (a statistical test that checks whether an operation's running time depends on secret data). Run it with:
